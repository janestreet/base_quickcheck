...
module Partitioning (Value : Partition_value) : sig
  ...
  let create' hash_list_fn =
    Map.of_alist_exn (module Value) (List.map Value.examples ~f:(fun value ->
      (value, hash_list_fn value)))
    |> of_hash_keys

  let create hash_fn =
    create' (fun value -> [hash_fn value])

  let union list =
    create' (fun value ->
      List.concat_map list ~f:(fun t ->
        Map.find_exn t.hash_keys value))

  let is_complete t =
    Set.for_all ~f:set_is_singleton t.partitions

  let is_singleton t =
    set_is_singleton t.partitions
end
...
let test_generator (type a) ?config ?(mode = `exhaustive) ?cr generator m =
  ...
  Test.with_sample_exn generator ?config ~f:(fun sequence ->
    ...
    match mode with
    | `exhaustive ->
      require [%here] ?cr (Set.is_empty failed_to_generate)
        ~if_false_then_print_s:(lazy [%message "failed to generate all known values"])
    | `inexhaustive ->
      require [%here] ?cr (not (Set.is_empty failed_to_generate))
        ~if_false_then_print_s:
          (lazy [%message
            "generated all known values even though we did not expect to"])
  )
...
let test_observer (type a) ?config ?(mode = `transparent) ?cr observer m =
  ...
  Test.with_sample_exn hash_fn_gen ?config ~f:(fun sample ->
    ...
    match mode with
    | `transparent ->
      require [%here] ?cr (List.exists partitionings ~f:Partitioning.is_complete)
        ~if_false_then_print_s:
          (lazy [%message
            "did not generate any single function that distinguishes all values"])
    | `opaque ->
      require [%here] ?cr (Partitioning.is_singleton partitions)
        ~if_false_then_print_s:
          (lazy [%message
            "generated functions did not treat input as opaque"])
  )
...
let test_shrinker (type a) ?config:_ ?(mode = `compound) ?cr shrinker m =
  ...
  match mode with
  | `atomic ->
    require [%here] ?cr (List.is_empty alist)
      ~if_false_then_print_s:
        (lazy [%message "atomic shrinker should not shrink values"])
  | `compound ->
    require [%here] ?cr (not (List.is_empty alist))
      ~if_false_then_print_s:
        (lazy [%message "compound shrinker should shrink values"])
...
let m_char
  : (module With_examples with type t = char)
  = (module struct
    include Char
...
let m_biject (type a) (type b) (module A : With_examples with type t = a) ~f ~f_inverse
...
let m_list (type elt) (module Elt : With_examples with type t = elt)
  : (module With_examples with type t = elt list)
  = (module struct
  ...
  = (module struct
    ...
    let examples =
      ...
      then begin
        ...
        List.map B.examples ~f:Fn.const
      end
  end)
...
let m_either (type a) (type b)
  ...
  = (module struct
    ...
    let examples =
      List.map A.examples ~f:Either.first @
      List.map B.examples ~f:Either.second
  end)

let m_result (type a) (type b)
      (module A : With_examples with type t = a)
      (module B : With_examples with type t = b)
  ...
  = (module struct
    ...
    let examples =
      List.map A.examples ~f:(fun x -> Ok x) @
      List.map B.examples ~f:(fun x -> Error x)
  end)

let m_pair (type a) (type b)
      (module A : With_examples with type t = a)
      (module B : With_examples with type t = b)
  : (module With_examples with type t = a * b)
  = (module struct
...
let m_string
  : (module With_examples with type t = string)
  = (module struct
...
let m_nat ~up_to : (module With_examples with type t = int)
  = (module struct
...
let m_nat' (type i) ~up_to (module I : Int.S with type t = i)
  : (module With_examples with type t = i)
  = (module struct
    type t = I.t [@@deriving compare, sexp_of]
    let examples =
      List.range 0 up_to ~start:`inclusive ~stop:`inclusive
      |> List.map ~f:I.of_int_exn
  end)
...
let m_float : (module With_examples with type t = float)
  = (module struct
...
let m_sexp : (module With_examples with type t = Sexp.t)
  = (module struct
    type t = Sexp.t [@@deriving compare, sexp_of]

    let examples =
      let atoms =
        List.map ~f:(fun string -> Sexp.Atom string) ["a"; "bc"; "def"]
      in
      let lists_of_atoms =
        List.map atoms ~f:(fun atom -> Sexp.List [atom])
        @ [Sexp.List atoms]
      in
      atoms @ lists_of_atoms @ [Sexp.List (atoms @ lists_of_atoms)]
  end)

let m_set (type elt) (type cmp)
...
let m_map (type key) (type data) (type cmp)
  ...
  : (module With_examples with type t = (key, data, cmp) Map.t)
  = (module struct
