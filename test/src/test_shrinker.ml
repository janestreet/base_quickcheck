...
let%expect_test "create & shrink" =
  List.init 10 ~f:(fun size ->
    (size, Shrinker.shrink natural_number_shrinker size))
...
let%expect_test "fixed_point" =
  test_shrinker
    (Shrinker.fixed_point (fun shrinker ->
       Shrinker.map (Shrinker.option shrinker)
         ~f:(function None -> 0 | Some n -> n + 1)
         ~f_inverse:(function 0 -> None | n -> Some (n - 1))))
    (m_nat ~up_to:4);
  [%expect {|
    (shrinker
     ((1 => 0)
