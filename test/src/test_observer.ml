...
let%expect_test "observe & create" [@tags "64-bits-only"] =
  ...
  List.init 10 ~f:(fun size ->
    Observer.observe obs 6 ~size ~hash:(Hash.alloc ())
    |> Hash.get_hash_value)
  |> [%sexp_of: int list]
  |> print_s;
  [%expect {|
...
let%expect_test "fn" =
  ...
  test_observer ~config
    (Observer.fn Generator.bool Observer.bool)
    first_order;
  [%expect {| (observer transparent) |}];
...
let%expect_test "both" =
  test_observer
    (Observer.both Observer.bool Observer.bool)
...
let%expect_test "either" =
  test_observer
    (Observer.either Observer.bool Observer.bool)
...
let%expect_test "result" =
  test_observer
    (Observer.result Observer.bool Observer.bool)
