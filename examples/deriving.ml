...
let add_rational x y =
  ...
  | _ ->
    let nx = numerator x in let dx = denominator x in
    let ny = numerator y in let dy = denominator y in
...
let%test_unit "commutativity" =
  Test.run_exn
    ...
    ~f:(fun (x, y) ->
      [%test_eq: number] (add_number x y) (add_number y x))
;;
...
let%test_unit "left identity" =
  Test.run_exn
    ...
    ~f:(fun x ->
      [%test_eq: number] x (add_number (Real (Rational (Integer zero))) x))
;;
...
let%test_unit "right identity" =
  Test.run_exn
    ...
    ~f:(fun x ->
      [%test_eq: number] x (add_number x (Real (Rational (Integer zero)))))
;;
