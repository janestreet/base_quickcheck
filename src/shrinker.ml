...
module T : sig
  ...
  let atomic = fun _ -> Sequence.empty
...
let map t ~f ~f_inverse =
  create (fun x ->
    Sequence.map ~f (shrink t (f_inverse x)))

let fixed_point of_shrinker =
  let rec f x =
    Sequence.of_lazy (lazy (shrink (of_shrinker (create f)) x))
  in
  create f
...
let sexp =
  fixed_point (fun shrinker ->
    create (function
      ...
      | Sexp.List l ->
        let shrink_list =
          shrink (list shrinker) l
          |> Sequence.map ~f:(fun l -> Sexp.List l)
        in
...
let result ok_t err_t =
  map (either ok_t err_t)
    ~f:(function First ok -> Ok ok | Second err -> Error err)
    ~f_inverse:(function Ok ok -> First ok | Error err -> Second err)
...
let map_t key_t data_t =
  create (fun map_t ->
    ...
    let t =
      map (map_tree_using_comparator ~comparator key_t data_t)
...
let set_t elt_t =
  create (fun set_t ->
    ...
    let t =
      map (set_tree_using_comparator ~comparator elt_t)
