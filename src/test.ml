...
let default_config : Config.t = {
  ...
  shrink_count = 10_000;
  sizes        =
    Sequence.cycle_list_exn (List.range 0 ~start:`inclusive 30 ~stop:`inclusive);
}

let lazy_nondeterministic_state =
...
let initial_random_state ~config =
  ...
  | Deterministic string ->
    Splittable_random.State.of_int (String.hash string)

let one_size_per_test ~(config : Config.t) =
  Sequence.unfold
    ~init:(config.sizes, 0)
    ~f:(fun (sizes, number_of_size_values) ->
...
let input_sequence ~config ~examples ~generator =
  ...
  Sequence.append
    ...
    (one_size_per_test ~config
     |> Sequence.map ~f:(fun size ->
       Generator.generate generator ~size ~random))
  ...
  with_sample M.quickcheck_generator ~config ~examples ~f:(fun sequence ->
    match
      Sequence.fold_result sequence ~init:() ~f:(fun () input ->
        match Or_error.try_with_join (fun () -> f input) with
        | Ok ()       -> Ok ()
...
let with_sample_exn ~f ?config ?examples generator =
  ...
  with_sample ~f ?config ?examples generator
  |> Or_error.ok_exn

let run_exn ~f ?config ?examples testable =
  let f x = Or_error.try_with (fun () -> f x) in
  run ~f ?config ?examples testable
  |> Or_error.ok_exn
