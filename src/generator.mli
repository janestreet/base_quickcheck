...
val%template map_t_m
  : 'key 'data 'cmp.
  ('key, 'cmp) Comparator.Module.t -> 'key t -> 'data t -> ('key, 'data, 'cmp) Map.t t
[@@mode p = (nonportable, portable)]

val%template set_t_m
  : 'elt 'cmp.
  ('elt, 'cmp) Comparator.Module.t -> 'elt t -> ('elt, 'cmp) Set.t t
[@@mode p = (nonportable, portable)]
...
val%template string_of : char t -> string t [@@mode p = (nonportable, portable)]
val%template string_non_empty_of : char t -> string t [@@mode p = (nonportable, portable)]

val%template string_with_length_of : char t -> length:int -> string t
[@@mode p = (nonportable, portable)]
