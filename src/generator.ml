...
let of_list list =
  ...
  then begin
    Error.raise_s [%message "Base_quickcheck.Generator.of_list: empty list"]
  end;
...
let of_weighted_list alist =
  ...
  then begin
    Error.raise_s [%message "Base_quickcheck.Generator.of_weighted_list: empty list"]
  end;
  ...
  fixed_point (fun self ->
    let rec_list =
      List.map (f self) ~f:(fun (w, t) ->
        (w, (let%bind n = size in with_size ~size:(n-1) t)))
    in
...
let option value_t =
  union [return None; map value_t ~f:Option.return]

let either fst_t snd_t =
  union [
    map fst_t ~f:Either.first;
    map snd_t ~f:Either.second;
  ]

let result ok_t err_t =
  map (either ok_t err_t)
    ~f:(function
      | First  ok  -> Ok    ok
...
let list_non_empty elt_gen =
  list_generic ~min_length:1 elt_gen
...
module For_integer (Integer : Int_with_random) = struct
  let uniform_inclusive lo hi =
    create (fun ~size:_ ~random ->
      Integer.uniform random ~lo ~hi)

  let log_uniform_inclusive lo hi =
    create (fun ~size:_ ~random ->
      Integer.log_uniform random ~lo ~hi)
  ...
  let uniform_all =
    uniform_inclusive Integer.min_value Integer.max_value

  let all =
    [%map
      let negative  = bool
      and magnitude = log_inclusive Integer.zero Integer.max_value
      in
      if negative
      then Integer.bit_not magnitude
      else magnitude]
end
...
let float_max_positive_subnormal_value = Float.one_ulp `Down Float.min_positive_normal_value
...
let float_zero =
  ...
  in
  Float.create_ieee_exn ~negative ~exponent:float_zero_exponent ~mantissa:float_zero_mantissa

let float_subnormal =
  let%map negative = bool
  and exponent = return float_subnormal_exponent
  and mantissa = For_int63.log_inclusive float_min_subnormal_mantissa float_max_subnormal_mantissa
  in
...
let float_normal =
  ...
  and mantissa = float_normal_mantissa
  in
  Float.create_ieee_exn ~negative ~exponent ~mantissa
...
let float_infinite =
  let%map negative = bool
  in
  Float.create_ieee_exn ~negative ~exponent:float_inf_exponent ~mantissa:float_inf_mantissa

let float_nan =
  let%map negative = bool
  and exponent = return float_nan_exponent
  and mantissa = For_int63.inclusive float_min_nan_mantissa float_max_nan_mantissa
  in
  Float.create_ieee_exn ~negative ~exponent ~mantissa
...
let float_matching_classes filter =
  List.filter_map Float.Class.all ~f:(fun c ->
    if filter c
    then Some (float_weight_of_class c, float_of_class c)
    else None)
  |> weighted_union
...
let float_uniform_exclusive lower_bound upper_bound =
  let open Float.O in
  if not (Float.is_finite lower_bound) || not (Float.is_finite upper_bound) then begin
    raise_s [%message
  ...
  if lower_inclusive > upper_inclusive then begin
    raise_s [%message
...
let sexp_of atom =
  fixed_point (fun self ->
    ...
    | 0 -> let%map atom = atom in Sexp.Atom atom
    (* relying on [List.gen] to distribute [size] over sub-sexps *)
    | _ -> let%map list = list self in Sexp.List list)
